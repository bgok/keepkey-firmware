...
void u2f_handle_enroll(u2f_service_t *service, uint8_t p1, uint8_t p2, uint8_t *buffer, uint16_t length) {	
	...
	memcpy(challengeParameter, buffer, 32);
	memcpy(applicationParameter, buffer + 32, 32);

	if (!service->userPresence) {
		 u2f_send_fragmented_response(service, U2F_CMD_MSG, (uint8_t*)SW_PROOF_OF_PRESENCE_REQUIRED, sizeof(SW_PROOF_OF_PRESENCE_REQUIRED), false);
		if (!service->promptUserPresence) {
			service->promptUserPresence = true;
			service->promptUserPresenceFunction(service, true, applicationParameter);
		}
	}
	else {
		uint16_t offset = 0;
		...
		keyHandleLength = u2f_crypto_generate_key_and_wrap(applicationParameter, service->messageBuffer + offset, service->messageBuffer + offset + 65 + 1);
		...
		offset += u2f_crypto_copy_attestation_certificate(service->messageBuffer + offset);
		// Compute signature
		if (!u2f_sign_init() || 
			!u2f_sign_update(DUMMY_ZERO, 1) ||
			!u2f_sign_update(applicationParameter, 32) ||
			!u2f_sign_update(challengeParameter, 32) ||
			!u2f_sign_update(service->messageBuffer + 1 + 65 + 1, keyHandleLength) ||
			!u2f_sign_update(service->messageBuffer + 1, 65)) {
			goto internal_error;					
		}
		signatureLength = u2f_crypto_sign_attestation(service->messageBuffer + offset);
...
void u2f_handle_sign(u2f_service_t *service, uint8_t p1, uint8_t p2, uint8_t *buffer, uint16_t length) {	
	(void)p2;
	uint8_t challengeParameter[32];
	uint8_t applicationParameter[32];
	...
	memcpy(keyHandle, buffer + 65, keyHandleLength);
	...
	// If the application parameter doesn't match the last validated one, reset the user presence
	if (service->confirmedApplicationParameter != NULL) {
		if (!compare_constantTime(applicationParameter, service->confirmedApplicationParameter, 32)) {
			service->userPresence = 0;		
		}
	}
	...
	if (!service->userPresence) {
		...
		if (!service->promptUserPresence) {
			service->promptUserPresence = true;
			if (service->confirmedApplicationParameter != NULL) {
				memcpy(service->confirmedApplicationParameter, applicationParameter, 32);
			}
			service->promptUserPresenceFunction(service, false, applicationParameter);
		}
	}	
	else {
		uint16_t offset = 0;
		...
		signatureLength = u2f_crypto_sign_application(service->messageBuffer + offset);
		if (signatureLength == 0) {
			goto internal_error;
		}
		offset += signatureLength;
...
void u2f_handle_get_version(u2f_service_t *service, uint8_t p1, uint8_t p2, uint8_t *buffer, uint16_t length) {	
	(void)p1;
	(void)p2;
	(void)buffer;
	(void)length;
...
void u2f_handle_cmd_init(u2f_service_t *service, uint8_t *buffer, uint16_t length) {
	uint8_t channel[4];
	(void)length;
	uint16_t offset = 0;
	random_buffer(channel, 4);
	memcpy(service->messageBuffer + offset, buffer, 8);
	offset += 8;
	memcpy(service->messageBuffer + offset, channel, 4);
	offset += 4;
	...
	u2f_send_fragmented_response(service, U2F_CMD_INIT, service->messageBuffer, offset, true);
...
void u2f_handle_cmd_msg(u2f_service_t *service, uint8_t *buffer, uint16_t length) {
	...
	if ((dataLength != (uint16_t)(length - 9)) && (dataLength != (uint16_t)(length - 7))) { // Le is optional
		u2f_response_error(service, ERROR_PROP_UNCONSISTENT_MSG_LENGTH, true);
		return;
	}
	if (cla != FIDO_CLA) {
...
void u2f_process_message(u2f_service_t *service, uint8_t *buffer) {
	...
	switch(cmd) {
		case U2F_CMD_INIT:
			u2f_handle_cmd_init(service, buffer + 3, length);
			break;
		case U2F_CMD_PING:
